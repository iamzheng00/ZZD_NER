...
class Model_bilstm(keras.Model):
    ...
    def inner_train_one_step(self, batches, epoch_num, ckpt_manager, log_writer=None):
        '''
        :param self:
        ...
        with tqdm(total=batch_size) as bar:
            ...
            with log_writer.as_default():
                ...
                tf.summary.scalar("loss", loss, step=epoch_num)
                tf.summary.scalar("P", P_t, step=epoch_num)
                tf.summary.scalar("R", R_t, step=epoch_num)
                tf.summary.scalar("F", F1_t, step=epoch_num)
